...
public class Mission implements TelecommandMessage {
	 ...
	 * @param waypoints
	 */
	public Mission(int missionID, OnConnectionLostMode onConnectionLostMode, List<Waypoint> waypoints) {
		super();
	...
	private void parseJSONWaypoints(JSONArray jsonWaypoints) {
		jsonWaypoints.forEach(waypoint -> {
			...
			waypoints.add(new Waypoint(wp.getString("type"), wp.getFloat("longitude"), wp.getFloat("latitude"),
					wp.getInt("altitude")));
		});
	}
	...
	public String toString() {
		final int maxLen = 10;
		return "Mission [missionID=" + missionID + ", onConnectionLostMode=" + onConnectionLostMode + ", waypoints="
